  ...
  while (n_left_from > 0)
    {
	  ...
	  // Send mapping to all fip64 specific nodes
          vnet_buffer (p0)->map_t.v6.saddr = ip4_mapping.src.as_u32;
          vnet_buffer (p0)->map_t.v6.daddr = ip4_mapping.dst.as_u32;
          ...
          if (PREDICT_FALSE ( p0->flags & VLIB_BUFFER_IS_TRACED ))
            {
              ...
              trace->ip4.src_address = ip4_mapping.src;
              trace->ip4.dst_address = ip4_mapping.dst;
          ...
          if (PREDICT_FALSE (vnet_buffer (p0)->map_t.v6.frag_offset &&
                ...
                (vnet_buffer (p0)->map_t.v6.l4_protocol == IP_PROTOCOL_TCP))
            {
              ...
              vnet_buffer (p0)->map_t.checksum_offset =
                vnet_buffer (p0)->map_t.v6.l4_offset + 16; //MIDOTODO: check this offset
