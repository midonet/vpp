...
ip6_fip64 (vlib_main_t * vm, vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  vlib_node_runtime_t *error_node =
    vlib_node_get_runtime (vm, ip6_fip64_node.index);
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from > 0 && n_left_to_next > 0)
    {
      ...
      {
        fip64_trace_t *trace = vlib_add_trace(vm, node, p0,
      ...
      else if (PREDICT_TRUE (vnet_buffer (p0)->map_t.v6.l4_protocol ==
                             IP_PROTOCOL_TCP))
      {
        error0 = l4_len0 < sizeof (tcp_header_t) ? FIP64_ERROR_MALFORMED :
                                                   error0;
        vnet_buffer (p0)->map_t.checksum_offset =
        vnet_buffer (p0)->map_t.v6.l4_offset + 16;
        next0 = IP6_FIP64_NEXT_MAPT_TCP_UDP;
        src_port0 =
          (i32) * ((u16 *) u8_ptr_add (ip60, vnet_buffer (p0)->map_t.v6.l4_offset));
      }
      else if (PREDICT_TRUE (vnet_buffer (p0)->map_t.v6.l4_protocol ==
                             IP_PROTOCOL_UDP))
      {
        error0 = l4_len0 < sizeof (udp_header_t) ? FIP64_ERROR_MALFORMED :
                                                   error0;
        vnet_buffer (p0)->map_t.checksum_offset =
      ...
      {
        error0 = l4_len0 < sizeof (icmp46_header_t) ? FIP64_ERROR_MALFORMED :
        error0;
        next0 = IP6_FIP64_NEXT_MAPT_ICMP;
        if (((icmp46_header_t *)
             u8_ptr_add (ip60, vnet_buffer (p0)->map_t.v6.l4_offset))->code ==
            ICMP6_echo_reply ||
            ((icmp46_header_t *)
             u8_ptr_add (ip60, vnet_buffer (p0)->map_t.v6.l4_offset))->code ==
            ICMP6_echo_request)
          src_port0 =
            (i32) * ((u16 *) u8_ptr_add (ip60,
                                         vnet_buffer (p0)->map_t.v6.l4_offset + 6));
      }
      else
      {
        // TODO: In case of 1:1 mapping, it might be possible to do something
        // with those packets.
...
ip6_get_port (ip6_header_t * ip6, fip64_dir_e dir, u16 buffer_len)
{
  ...
  {
    return (dir == FIP64_SENDER) ? ((udp_header_t *) (l4))->src_port :
                                   ((udp_header_t *) (l4))->dst_port;
  }
                 ...
                 u8 * error)
{
  ...
  ip4->length =
    u16_net_add (ip6->payload_length,
                 (inner_ip6 == NULL) ? sizeof (*ip4) :
                                       (2 * sizeof (*ip4) - sizeof (*ip6)));
  ip4->checksum = ip4_header_checksum (ip4);

  //TODO: We could do an easy diff-checksum for echo requests/replies
  //Recompute ICMP checksum
  icmp->checksum = 0;
  csum = ip_incremental_checksum (0, icmp,
...
static uword
ip6_fip64_icmp (vlib_main_t * vm, vlib_node_runtime_t * node,
                vlib_frame_t * frame)
{
  u32 n_left_from, *from, next_index, *to_next, n_left_to_next;
  vlib_node_runtime_t *error_node =
