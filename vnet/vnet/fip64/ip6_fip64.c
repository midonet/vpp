...
#define IP4_DST_ADDRESS 0x0A000001
#define IP4_SRC_ADDRESS 0x64000001

typedef enum
{
  IP6_FIP64_NEXT_FIP64_ICMP,
  IP6_FIP64_NEXT_DROP,
  IP6_FIP64_N_NEXT
} fip64_ip6_next_t;
...
fip64_get_ip4 (ip6_address_t *addr)
{
...
_ip6_fip64_icmp (vlib_main_t *vm, vlib_node_runtime_t *node,
                vlib_buffer_t * p, u8 * error)
{
  ...
  ip4_sadr = clib_host_to_net_u32(IP4_SRC_ADDRESS);
  ...
  if (inner_ip6)
  {
    ...
    inner_ip4_dadr = fip64_get_ip4 (&inner_ip6->dst_address);
    inner_ip4->dst_address.as_u32 = inner_ip4_dadr;
    inner_ip4->src_address.as_u32 = clib_host_to_net_u32(10 << 24 | 1);
  ...
  ip4->dst_address.as_u32 = clib_host_to_net_u32(IP4_DST_ADDRESS);
...
ip6_fip64 (vlib_main_t * vm, vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from > 0 && n_left_to_next > 0)
    {
      ...
      frag0 = (ip6_frag_hdr_t *) u8_ptr_add (ip60,
                                             vnet_buffer (p0)->map_t.v6.frag_offset);

      if (PREDICT_FALSE (vnet_buffer (p0)->map_t.v6.frag_offset &&
                         ip6_frag_hdr_offset (frag0)))
      {
        // TODO: Add support for fragmented packets.
        error0 = FIP64_ERROR_MALFORMED;
        next0 = IP6_FIP64_NEXT_DROP;
      }
      else
      {
        // TODO: Add support for TCP/UDP.
        if (vnet_buffer (p0)->map_t.v6.l4_protocol ==
            IP_PROTOCOL_ICMP6)
        {
          error0 =
            l4_len0 <
            sizeof (icmp46_header_t) ? FIP64_ERROR_MALFORMED : error0;
          next0 = IP6_FIP64_NEXT_FIP64_ICMP;
        }
        else
        {
...
VLIB_REGISTER_NODE(ip6_fip64_node) = {
  ...
  .next_nodes = {
      [IP6_FIP64_NEXT_FIP64_ICMP] = "ip6-fip64-icmp",
