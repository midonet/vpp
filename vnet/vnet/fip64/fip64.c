...
specified for VRF %d: %x", tenant->table_id, tenant->vni);
    }
  }
...
\t\t\t\t pool can be omitted if there is already one specified for given table; default table is 0. vni is expected in hex format",
