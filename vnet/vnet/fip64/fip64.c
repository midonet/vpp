...
uword *ip6_ip4_hash; /* (ip6 address, ip4 address) hash */
uword *ip4_ip6_hash; /* (ip4 address, ip6 address) hash */

static clib_error_t *
fip64_init (vlib_main_t * vm)
{
  ip6_ip4_hash = hash_create_mem(0, sizeof(ip6_address_t), sizeof(ip4_address_t));
  ip4_ip6_hash = hash_create_mem(0, sizeof(ip4_address_t), sizeof(ip6_address_t));
  return 0;
}
...
{
  hash_set_mem(ip6_ip4_hash, ip6, ip4);
  hash_set_mem(ip4_ip6_hash, ip4, ip6);
  return 0;
}
...
fip64_delete_mapping(ip6_address_t * ip6)
{
  ...
  hash_unset(ip6_ip4_hash, stored_ip6);
  hash_unset(ip4_ip6_hash, stored_ip4);
  free(stored_ip4);
  free(stored_ip6);
  return 0;
}
...
{
  uword * p = hash_get_mem(ip6_ip4_hash, ip6);
...
{
  uword * p = hash_get_mem(ip4_ip6_hash, ip4);
