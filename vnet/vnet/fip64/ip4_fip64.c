...
_ip4_fip64_icmp (vlib_buffer_t * p, u8 * error)
{
  ...
  ip4_fip64_pseudo_header_t *pheader = vlib_buffer_get_current (p);
                   ...
                   vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from > 0 && n_left_to_next > 0)
    {
      ...
      ip4_fip64_pseudo_header_t *pheader = vlib_buffer_get_current (p0);
...
ip4_fip64 (vlib_main_t * vm, vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from > 0 && n_left_to_next > 0)
    {
      ...
      ip4_fip64_pseudo_header_t *pheader0 = vlib_buffer_get_current(p0);
